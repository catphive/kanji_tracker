(function (_) {
    ...
    function KanjiMatch(text, kanjiSet) {
        var self = this;
        var textKanji = uniqueKanji(text);

        self.kanjis = kanjiSet.kanjis;
        self.displayName = kanjiSet.displayName;

        self.matches = _.filter(kanjiSet.kanjis, function (kanji) {
            return kanji in textKanji;
        }).join('');

        self.misses = _.filter(kanjiSet.kanjis, function (kanji) {
            return !(kanji in textKanji);
        }).join('');
    }

    function matchKanjiSets(text, kanjiSets) {
        return _.map(kanjiSets, function (kanjiSet) {
            return new KanjiMatch(text, kanjiSet);
        });
    }

    function aggregateMatches(kanjiMatches) {
        return {
            kanjis: _.pluck(kanjiMatches,'kanjis').join(''),
            matches: _.pluck(kanjiMatches, 'matches').join(''),
            misses: _.pluck(kanjiMatches, 'misses').join('')
        };
    }

    var countApp = angular.module('countApp', []);

    countApp.controller('FileController',  function ($scope) {

        // Listen for changes on files, read them in, and update scope.
        $scope.$watch('files', function (files) {
            var texts = [];
            _.each(files, function (file) {

                // This won't work on IE < 10. Need a shim.
                var reader = new FileReader();
                reader.onload = function (loadEvt) {
                    texts.push(loadEvt.target.result);

                    // Once all files loaded, concat.
                    if (files.length === texts.length) {
                        $scope.$apply(function () {
                            $scope.kanjiMatches = matchKanjiSets(
                                texts.join(''),
                                kanjiSets
                            );
                            $scope.aggregateMatches = aggregateMatches(
                                $scope.kanjiMatches
                            );
                        });
                    }
    ...
    countApp.directive('fileinput', function () {
        return {
            template: '<input type="file" multiple="true"/>',
            replace: true,
            restrict: 'E',
            require: '?ngModel',
