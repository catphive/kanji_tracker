(function ($, _) {
    ...
    function procGrade(grade, gradeList, kanjiSet) {
        var matches = _.filter(gradeList, function (kanji) {
            return !(kanji in kanjiSet);
        });
        console.log(matches);

        var unlearnedCount = matches.length;
        $('#' + grade + '-count').text(unlearnedCount);
        $('#' + grade + '-total').text(gradeList.length);

        var $kanji = $('#' + grade + '-kanji');
        $kanji.html('');
        matches.forEach(function (kanji) {
            $kanji.append('<span class="char">' + kanji + '</span>');
            $kanji.append(' ');
        });
        return unlearnedCount;
    }

    function procText(kanjiSet) {
        var kanjis = _.keys(kanjiSet);
        $('#count').text(kanjis.length);
    ...
    $('#files').change(function (evt) {
        ...
        reader.onload = function (loadEvt) {
            var kanjiSet = uniqueKanji(loadEvt.target.result);
            procText(kanjiSet);
            var unlearnedCount = 0;
            unlearnedCount += procGrade('grade1', grade1, kanjiSet);
            unlearnedCount += procGrade('grade2', grade2, kanjiSet);
            unlearnedCount += procGrade('grade3', grade3, kanjiSet);
            unlearnedCount += procGrade('grade4', grade4, kanjiSet);
            unlearnedCount += procGrade('grade5', grade5, kanjiSet);
            unlearnedCount += procGrade('grade6', grade6, kanjiSet);
            unlearnedCount += procGrade('grade7to12', grade7to12, kanjiSet);
            unlearnedCount += procGrade('additions', additions, kanjiSet);

            $('#total-unlearned').text(unlearnedCount.toString());
        };
